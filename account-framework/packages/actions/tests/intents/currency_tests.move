#[test_only]
module account_actions::CurrencyObj;
...
use account_actions::{
    ...
    currency_intents as acc_currency_intents,
...
public struct CurrencyObj has key { id: UID }
...
fun start(): (
    ...
    // create TreasuryCap, Currency and MetadataCap
...
fun test_request_execute_disable_rules() {
    ...
    acc_currency_intents::request_disable_rules<Config, Outcome, CurrencyObj>(
        auth,
        &mut account,
        params,
        outcome,
    ...
    acc_currency_intents::execute_disable_rules<Config, Outcome, CurrencyObj>(&mut executable, &mut account);
    ...
    let lock = acc_currency::borrow_rules<Config, CurrencyObj>(&account);
...
fun test_request_execute_mint_and_keep() {
    ...
    acc_currency_intents::request_mint_and_transfer<Config, Outcome, CurrencyObj>(
        auth,
        &mut account,
        params,
        outcome,
    ...
    let (_, mut executable) = account.create_executable<_, Outcome, _>(key, &clock, version::current(), DummyIntent());
    acc_currency_intents::execute_mint_and_transfer<_, Outcome, CurrencyObj>(&mut executable, &mut account, scenario.ctx());
    account.confirm_execution(executable);
...
fun test_request_execute_mint_and_keep_with_max_supply() {
    let (mut scenario, extensions, mut account, clock, cap, currency, metadata_cap) = start();
    let auth = account.new_auth(version::current(), DummyIntent());
    acc_currency::lock_caps(auth, &mut account, cap, option::some(metadata_cap), option::some(5));
...
fun test_request_execute_withdraw_and_burn() {
    ...
    account.keep(coin);
    scenario.next_tx(OWNER);
    let receiving = ts::most_recent_receiving_ticket<Coin<CurrencyObj>>(&object::id(&account));
    ...
    acc_currency_intents::request_withdraw_and_burn<Config, Outcome, CurrencyObj>(
        auth,
        &mut account,
        params,
        outcome,
    ...
    let (_, mut executable) = account.create_executable<_, Outcome, _>(key, &clock, version::current(), DummyIntent());
    acc_currency_intents::execute_withdraw_and_burn<_, Outcome, CurrencyObj>(
        &mut executable,
        &mut account,
        vector[receiving],
        scenario.ctx()
    ...
    let mut expired = account.destroy_empty_intent<_, Outcome>(key);
    owned::delete_withdraw_coin<_, CurrencyObj>(&mut expired, &mut account);
...
fun test_request_execute_update_metadata() {
    ...
    acc_currency_intents::request_update_metadata<Config, Outcome, CurrencyObj>(
        auth,
        &mut account,
        params,
        outcome,
    ...
    let (_, mut executable) = account.create_executable<_, Outcome, _>(key, &clock, version::current(), DummyIntent());
    acc_currency_intents::execute_update_metadata<Config, Outcome, CurrencyObj>(&mut executable, &mut account, &mut currency);
    account.confirm_execution(executable);
...
fun test_request_execute_mint_and_transfer() {
    ...
    scenario.next_tx(OWNER);
    let coin1 = scenario.take_from_address<Coin<CurrencyObj>>(@0x1);
    assert!(coin1.value() == 1);
    let coin2 = scenario.take_from_address<Coin<CurrencyObj>>(@0x2);
    assert!(coin2.value() == 2);
    let coin3 = scenario.take_from_address<Coin<CurrencyObj>>(@0x3);
...
fun test_request_execute_mint_and_vest() {
    ...
    acc_currency_intents::request_mint_and_vest<Config, Outcome, CurrencyObj>(
        auth,
        &mut account,
        params,
        outcome,
    ...
    let (_, mut executable) = account.create_executable<_, Outcome, _>(key, &clock, version::current(), DummyIntent());
    acc_currency_intents::execute_mint_and_vest<_, Outcome, CurrencyObj>(&mut executable, &mut account, scenario.ctx());
    account.confirm_execution(executable);
    ...
    scenario.next_tx(OWNER);
    let stream = scenario.take_shared<Vesting<CurrencyObj>>();
...
#[test, expected_failure(abort_code = acc_currency_intents::EAmountsRecipentsNotSameLength)]
...
#[test, expected_failure(abort_code = acc_currency_intents::EMaxSupply)]
...
#[test, expected_failure(abort_code = acc_currency_intents::ECannotUpdateName)]
...
#[test, expected_failure(abort_code = acc_currency_intents::ECannotUpdateDescription)]
...
#[test, expected_failure(abort_code = acc_currency_intents::ECannotUpdateIcon)]
...
#[test, expected_failure(abort_code = acc_currency_intents::EBurnDisabled)]
...
#[test, expected_failure(abort_code = acc_currency_intents::EMintDisabled)]
